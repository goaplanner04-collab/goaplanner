...
export async function sendItineraryEmail({ to, itineraryText, userArea, shareId }) {
  ...
  const html = shell(`
    ...
    ${userArea ? `<p style="margin:0 0 18px;color:#9aa3b2;font-size:13px;">📍 ${userArea}</p>` : ""}
    ...
    <!-- THEN DASHBOARD CTA BELOW -->
    <div style="margin-top:22px;text-align:center;">
      <a href="${SITE_URL}/dashboard" style="display:inline-block;background:linear-gradient(135deg,#FF3D81,#FF6B6B);color:#fff;padding:13px 24px;border-radius:11px;text-decoration:none;font-weight:700;font-size:14px;">
        Open Your Dashboard →
      </a>
    ...
    <p style="margin:22px 0 0;color:#9aa3b2;font-size:12px;line-height:1.55;">
      Note: opening hours change. Confirm by phone or Google Maps before you head out, especially for late-night spots. Water-sports quotes are ~30% above the final — always negotiate.
    </p>
  `);
