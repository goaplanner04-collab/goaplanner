...
export default function PaywallModal({ open, onClose, initialPlan = "week" }) {
  ...
  useEffect(() => { if (open) loadRazorpayScript(); }, [open, loadRazorpayScript]);
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      rzp.on("payment.failed", () => { setError("Payment failed. Please try again."); setProcessing(false); });
      rzp.open();
    } catch {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(8px)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowY: "auto" }}
      onClick={(e) => { if (e.target === e.currentTarget && !processing) onClose?.(); }}
    >
      <div
        className="glass-card animate-slideIn"
        style={{ width: "100%", maxWidth: 520, padding: 24, background: "linear-gradient(180deg,rgba(20,20,30,0.95),rgba(10,10,15,0.98))", border: "1px solid rgba(255,45,120,0.3)", boxShadow: "0 0 60px rgba(255,45,120,0.25)", position: "relative", maxHeight: "95vh", overflowY: "auto" }}
      >
        <button
          onClick={() => !processing && onClose?.()}
          aria-label="Close"
          style={{ position: "absolute", top: 12, right: 12, background: "rgba(255,255,255,0.06)", border: "1px solid var(--border-glass)", color: "#fff", width: 36, height: 36, borderRadius: "50%", cursor: processing ? "not-allowed" : "pointer", fontSize: 18 }}
        >×</button>

        <h2 style={{ fontSize: 34, margin: "4px 0 4px", color: "var(--neon-pink)", textShadow: "0 0 20px rgba(255,45,120,0.5)" }}>
          UNLOCK GOANOW
        </h2>
          ...
          {Object.values(plans).map((p) => {
            ...
            return (
              <button
                ...
                style={{ background: active ? "linear-gradient(135deg,rgba(255,45,120,0.2),rgba(255,45,120,0.08))" : "rgba(255,255,255,0.03)", border: active ? "1.5px solid var(--neon-pink)" : "1px solid var(--border-glass)", borderRadius: 14, padding: "14px 8px", cursor: "pointer", color: "#fff", transition: "all 0.2s ease", position: "relative", boxShadow: active && p.popular ? "0 0 24px rgba(255,45,120,0.4)" : "none" }}
              >
                {p.popular && (
                  <div style={{ position: "absolute", top: -10, left: "50%", transform: "translateX(-50%)", background: "var(--neon-pink)", color: "#fff", fontSize: 9, padding: "2px 8px", borderRadius: 999, fontWeight: 700, letterSpacing: "0.05em" }}>
                    POPULAR
                  </div>
                )}
                <div style={{ fontSize: 11, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.08em" }}>{p.name}</div>
                <div style={{ fontFamily: "'Bebas Neue'", fontSize: 30, margin: "4px 0" }}>₹{p.price}</div>
        ...
        <ul style={{ listStyle: "none", padding: 0, margin: "0 0 20px", fontSize: 14, lineHeight: 1.7 }}>
          {["Nearest cafes & restobars by real GPS distance", "Live party & event feed with real crowd arrival times", "AI itinerary tailored to your exact vibe and budget", "Water sports, casinos, hidden gems — real pricing", "Works across all of Goa — Morjim to Palolem"].map((b) => (
        ...
        {error && (
          <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.4)", color: "#fca5a5", padding: "10px 14px", borderRadius: 10, fontSize: 13, marginBottom: 12 }}>
            {error}
          </div>
        )}
        ...
        <div style={{ marginTop: 20, borderTop: "1px solid var(--border-glass)", paddingTop: 16 }}>
          <button
            type="button"
            onClick={() => setShowTrial((s) => !s)}
            style={{ background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", fontSize: 13, padding: 0, textDecoration: "underline" }}
          >
            {showTrial ? "▲ Hide" : "▼ Have a trial key?"}
          </button>
